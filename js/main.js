/******/ (() => {
	...
	(() => {
		...
		var phone_input = __webpack_require__(279); // CONCATENATED MODULE: ./node_modules/gsap/gsap-core.js
  ...
  */
		...
		var Observer_gsap,
			...
			_getTarget = function _getTarget(t, self) {
				return (
					((self && self._ctx && self._ctx.selector) || Observer_gsap.utils.toArray)(t)[0] ||
		...
		var ScrollTrigger_gsap,
			...
			_parseClamp = function _parseClamp(value, type, self) {
				var clamp =
					ScrollTrigger_isString(value) && (value.substr(0, 6) === "clamp(" || value.indexOf("max") > -1);
				self["_" + type + "Clamp"] = clamp;
				return clamp ? value.substr(6, value.length - 7) : value;
			},
			_keepClamp = function _keepClamp(value, clamp) {
				return clamp && (!ScrollTrigger_isString(value) || value.substr(0, 6) !== "clamp(")
					? "clamp(" + value + ")"
					: value;
			},
			...
			_getViewportDimension = function _getViewportDimension(dimensionProperty) {
				return (
					(dimensionProperty === "Height" ? _100vh : ScrollTrigger_win["inner" + dimensionProperty]) ||
			...
			_maxScroll = function _maxScroll(element, _ref2) {
				...
				return Math.max(
					...
					(s = "scroll" + d2) && (a = _getProxyProp(element, s))
						...
						: ScrollTrigger_isViewport(element)
						? (ScrollTrigger_docEl[s] || ScrollTrigger_body[s]) - _getViewportDimension(d2)
			...
			ScrollTrigger_parsePosition = function _parsePosition(
				...
				if (ScrollTrigger_isString(value) && value.substr(0, 3) === "max") {
					value =
						scrollerMax + (value.charAt(4) === "=" ? _offsetToPx("0" + value.substr(3), scrollerSize) : 0);
				}
				...
				self.refresh = function (soft, force, position, pinOffset) {
					...
					if (ScrollTrigger_isString(parsedEnd) && !parsedEnd.indexOf("+=")) {
						if (~parsedEnd.indexOf(" ")) {
							parsedEnd =
								(ScrollTrigger_isString(parsedStart) ? parsedStart.split(" ")[0] : "") + parsedEnd;
						} else {
					...
					if (pin) {
						...
						if (!max && end > 1) {
							...
							if (
								isViewport &&
								_getComputedStyle(ScrollTrigger_body)["overflow" + direction.a.toUpperCase()] !==
									"scroll"
							) {
			...
			_getScrollNormalizer = function _getScrollNormalizer(vars) {
				...
				vars.onPress = function () {
					...
					scale = ScrollTrigger_round(
						((ScrollTrigger_win.visualViewport && ScrollTrigger_win.visualViewport.scale) || 1) /
							initialScale
					);
					tween.pause();
					prevScale !== scale &&
		...
		ScrollTrigger_getGSAP() && ScrollTrigger_gsap.registerPlugin(ScrollTrigger_ScrollTrigger); // CONCATENATED MODULE: ./src/js/components/running-line.js
